...
export default {
  install (Vue, options) {
    Vue.prototype.$export = {
      ...
      /*
      * desc: 导出 excel
      * params preHeader: 用来在表头上方加入描述文字等(数据格式：[[]] )
      ...
      * */
      excel(params) {
        return new Promise((resolve, reject) => {
          ...
          let data = _params.data.map(row => _params.columns.map(col => row[col.prop] || ''))
          data.unshift(header)
          if (_params.preHeader && _params.preHeader.length > 0) {
            data = _params.preHeader.concat(data)
          }
          // 导出
          Excel.export_json_to_excel(data, _params.title)
          // 完成
          resolve()
        })
      },
