...
export default {
  ...
  methods: {
    ...
    initHash () {
      ...
      this.nickname = hashParams.nickname || ''
      if (this.nickname) {
        this.$route.meta.title = `${this.nickname}的访客列表`
      }
    ...
    getVisitorUserList () {
      ...
      this.tableInfo.list = []
      let params = JSON.parse(JSON.stringify(this.params))
      params.business_card = this.id
      this.$api.skill.getSalesmanVisitorUserList(params)
        .then((data) => {
